...
BootcampSchema.pre('deleteOne', { virtuals: true }, async function (next) {
  ...
  console.log(`Courses being removed from bootcamp ${bootcamp.name}`);
  await mongoose.model('Course').deleteMany({ bootcamp: this._conditions._id });
  next();
});

// Reverse populate with virtuals. we need an array of courses for each bootcamp
